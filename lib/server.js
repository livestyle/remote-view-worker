	...
	.on('connect', function(req, socket, head) {
		...
		if (session) {
			...
			socket.once('data', function(chunk) {
				var len = TUNNEL_READY.length;
				if (chunk.length < len || !chunk.slice(0, len).equals(TUNNEL_READY)) {
					debug('socket received invalid handshake, aborting');
					this.destroy(new Error('Unexpected handshake message'));
				} else {
					debug('socket ready');
					this.emit('ready');
				}
