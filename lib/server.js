...
var openedSockets = [];
var defaultOptions = {
	port: process.env.RV_WORKER_PORT || 9001
};
...
module.exports = function(options, callback) {
	...
	server.listen(options.port, 10000, function() {
...
function handleLiveStyleChannelRequest(req) {
	...
	sessionManager.getSession(sessionId, stats).then(
		function(session) {
			...
			session.liveStyleChannel = req.accept(null, `http://${session.data.publicId}, ${session.data.localSite}`);
		}, 
		function() {
