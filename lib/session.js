...
module.exports = class Session extends EventEmitter {
	constructor(data, options) {
		...
		this._livestyleTunnel = null;
		this._livestyleTunnelDest = null;
	 ...
	 * @param {Socket} socket
	 */
	set livestyleTunnel(socket) {
		this._livestyleTunnel = socket;
		var self = this;
		socket.once('close', function() {
			if (!this.destroyed) {
				this.destroy();
			}
			if (self._livestyleTunnel === this) {
				self._livestyleTunnel = null;
			}
		});
	}

	get livestyleTunnel() {
		return this._livestyleTunnel;
	}
	...
	livestyleRedirect(req, res, url) {
		var socket = this.livestyleTunnel;
		if (!socket) {
			return res.destroy();
		}

		socket.unpipe();
		if (this._livestyleTunnelDest) {
			this._livestyleTunnelDest.destroy();
		}

		this._livestyleTunnelDest = res;

		socket.write(headersPayload(req, parseUrl(url).host));
		socket.pipe(res).pipe(socket);
	}
	...
	destroy() {
		...
		if (this.livestyleTunnel) {
			this.livestyleTunnel.destroy();
		}
