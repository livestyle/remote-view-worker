...
class Session extends EventEmitter {
	 ...
	 */
	redirect(req, res, bounce) {
		if (this.sockets.length) {
			var url = urlUtils.parse(this.data.localSite);
			debug('bouncing to %s', url.host);
			// req.connection.pipe(inject('request'));
			// res.pipe(inject('response'));

			bounce({
				stream: this.sockets.shift(),
				headers: {
					'host': url.host,
					'x-forwarded-proto': url.protocol.replace(/:.*$/, '')
				}
			}).pipe(inject()).pipe(res);
		} else {
			this.addToQueue(req, res, bounce);
		}
	}
