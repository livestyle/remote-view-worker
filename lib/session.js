...
var defaultOptions = {
	...
	injectCode: '<script src="http://l.livestyle.io/livestyle-rv.js" data-livestyle-connect="ws://%publicId%/__livestyle__/"></script>'
};
...
module.exports = class Session extends EventEmitter {
	...
	_redirect(socket, req, res, head) {
		...
		if (req.headers['upgrade']) {
			...
			.pipe(tunnel).on('error', function(err) {
				if (!res.headersSent) {
					res.writeHead(500);
				}
				res.end(`Unable to process request to ${req.url}:\n${err.message}`);
				console.error(utils.dateMark(), 'Unable to process request to', req.url);
				console.error(err);
			})
	...
	_nextInQueue() {
		if (this._queue.length) {
			...
			// validate response connection: if it’s still available, create 
			// redirect, otherwise try next queued connection
			if (item[1].connection && item[1].connection.writable) {
				item[1].connection.removeListener('close', this.disposeQueued);
				this.redirect.apply(this, item);
			} else {
				process.nextTick(() => this._nextInQueue());
			}
		}
	}
