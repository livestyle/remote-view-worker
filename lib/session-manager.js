...
function getPublicIdFromHost(host) {
	...
	// publicId is first three domain parts (there’s might be more subdomains)
	parsed = parts.slice(-3).join('.');
...
function queryBySessionId(collection, sessionId) {
	...
	return new Promise(function(resolve, reject) {
		collection.findOne({_id: new ObjectID(sessionId), active: true}, function(err, doc) {
			if (err) {
